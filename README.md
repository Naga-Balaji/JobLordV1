...
| [Job Hunt Tracker (standalone).html](Job%20Hunt%20Tracker%20(standalone).html) | Global standalone dashboard — place once at `~/Downloads/` and point it at the output folder (`~/Downloads/JobLord/`) to read/write `tracker.json` (tabs: Easy Apply · Direct/Company · Applied Log). |
...
Written to `~/Downloads/JobLord/` (the stable `output_root` set in [JobLord_agent.md](JobLord_agent.md) → Config). Cumulative memory lives at the root; each run gets its own dated subfolder. The dashboard is a **global viewer** placed one level up at `~/Downloads/` and loads the output folder — it is not a per-run output.

```
~/Downloads/
├── Job Hunt Tracker (standalone).html   # GLOBAL dashboard — loads the output folder below
└── JobLord/                             # output_root (stable across runs)
    ...
    └── <YYYY-MM-DD>-JobLord/            # per-run folder
